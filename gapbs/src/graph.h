...
class CSRGraph {
 ...
 public:
  ...
  std::vector<NodeID_> FindCommonNeighborIDsNoConstraints(NodeID_ v1, NodeID_ v2) const {

    std::vector<NodeID_> common_neighbors;

    NodeID_* v1_neighbors = new NodeID_[out_degree(v1)];
    NodeID_* v2_neighbors = new NodeID_[out_degree(v2)];

    int v1_n_size = 0, v2_n_size = 0;
    for(auto v1_n : out_neigh(v1)) {
      v1_neighbors[v1_n_size] = v1_n;
      v1_n_size++;
    }
    for(auto v2_n : out_neigh(v2)) {
      v2_neighbors[v2_n_size] = v2_n;
      v2_n_size++;
    }

    int i=0, j=0;
    while(i<v1_n_size && j<v2_n_size) {
   ...
   std::vector<NodeWeight<int, int> > SetIntersectionNeighborhood(std::vector<NodeWeight<int, int> > set, NodeID_ v, int threshold) const {

    std::vector<NodeWeight<int, int> > intersection_set;

    NodeWeight<int, int> * v_neighbors = new NodeWeight<int, int> [out_degree(v)];

    size_t v_n_size = 0;
    for(auto v_n : out_neigh(v)) {
      v_neighbors[v_n_size] = v_n;
      v_n_size++;
    }

    size_t i=0, j=0;
    while(i<set.size() && j<v_n_size) {
      if(set[i].v < v_neighbors[j].v) {
        i++;
      } else if(set[i].v > v_neighbors[j].v) {
        j++;
      } else {
        
        if(v_neighbors[j].w >= threshold && set[i].w >= threshold)
          intersection_set.push_back(set[i]);
        i++;
        j++;
      }
    }
  ...
  std::vector<NodeWeight<int, int> > FindCommonNeighborID_weightedEdge(NodeID_ v1, NodeID_ v2, int threshold=-1) const {
    std::vector<NodeWeight<int, int> > common_neighbors;

    NodeWeight<int, int> * v1_neighbors = new NodeWeight<int, int> [out_degree(v1)];
    NodeWeight<int, int> * v2_neighbors = new NodeWeight<int, int> [out_degree(v2)];
    ...
    int i=0, j=0;
    while(i<v1_n_size && j<v2_n_size) {
      if(v1_neighbors[i].v < v2_neighbors[j].v) {
        i++;
      } else if(v1_neighbors[i].v > v2_neighbors[j].v) {
        j++;
      } else {
        if(v1_neighbors[i].w >= threshold && v2_neighbors[j].w >= threshold)
          common_neighbors.push_back(v1_neighbors[i]);
        i++;
        j++;
      }
