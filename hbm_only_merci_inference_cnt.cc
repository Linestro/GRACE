...
int main( int argc, char* argv[] )
{
    ...
    std::ofstream out("hbm_only_merci_log/" + (prefix_group[prefix_idx]).first + ".log");
    ...
    if ((prefix_group[prefix_idx]).second == 0){
        ...
        graphfile.open("./MERCI/cluster/" + workload + "_patoh_hbm_1.0.cluster", std::ios::app);
    }
    ...
    cout << "MERCI access: " << post_compression_hbm + post_compression_ddr << endl;
