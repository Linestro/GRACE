...
for i in range(0, len(workloads)):
    ...
    metis_file_name = "../hbm_only_metis_log/" + workload + "_patoh_hbm_1.0.log"
    f = open(metis_file_name, "r")
    for line in f.readlines():
        if "Reduction Factor is:" in line:
            metis_arr.append(1. / float(line.split()[-1]))
...
f = open("fig10.txt", "w")
for i in range(0, len(workloads)):
    f.write(abbrev[i] + '\t' + str(1) + '\t' + str(metis_arr[i]) + '\t' + str(space_arr[i]) \
                 + '\t' + str(merci_arr[i]) + '\t' + str(grace_arr[i]) + '\t' + str(0.50) + '\n')  # Base is 1, oracle-of-2 is 0.50.

f.write('GM' + '\t' + str(1) + '\t' + str(gmean(metis_arr)) + '\t' + str(gmean(space_arr)) \
...
p0 = plt.bar(ind+width, WIN2, width,  align='center',
             color='#fbb4b9', edgecolor="black", label="Metis")
p1 = plt.bar(ind+2*width, WIN3, width, align='center',
             color='#f768a1', edgecolor="black", label="SPACE")
p2 = plt.bar(ind+3*width, WIN4, width, align='center',
             color='#c51b8a', edgecolor="black", label="MERCI")
p3 = plt.bar(ind+4*width, WIN5, width, align='center',
             color='#b2df8a', edgecolor="black", label="GRACE")
p4 = plt.bar(ind+5*width, WIN6, width, align='center',
...
plt.text(ind[-1]+4.65*width, WIN6[-1]+0.025,
         "{:.2f}".format(WIN6[-1]), fontsize=graph_font_size-23, rotation=90)
